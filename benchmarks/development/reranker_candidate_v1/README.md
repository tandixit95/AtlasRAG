...
The real model-backed profiler has now been executed on an uncontended RTX 4060 under the frozen protocol. The exact raw artifact is [`PROFILE.json`](PROFILE.json). It nominated the pre-frozen default depth 10 with batch size 32; this remains development evidence only and does not establish natural-workload quality, a production SLO, or default promotion. A separately frozen final evaluation is required before any promotion decision.
